...
log using "/Users/nithkosal/Dropbox/Research Projects/HouseholdIncome/empirical/CSES2017/Codes/cses2017_aux", replace text

/*******************************

cses2017_main_text.do
...
(3) Graph of average APC in each bin of URE, NNP, INC
...
quietly{
	...
	noi di " "
	noi tabstat NY NC NB ND NURE Nnom_asset Nnom_liab NNNP NINC APCn [aw=hweight], stat(mean sd) col(stat) format(%5.2f)
	noi di " "
...
****
***** (3) Percentile plot of APC against each exposure measure
****
...
foreach var in NURE NNNP NINC {
	...
	*Obtain mean value of APC and exposure measure at each percentile
	collapse APCn NURE NNNP NINC [pw=hweight], by(x`var') // Note: necessary to collapse all 3 exposure measures and not just the one being analyzed in `var' to allow at the end of the program "if `var'==NURE"..else and distinguish among maturities only for URE
	...
	* Plot and save scatter 
	...
	grstyle set color #ff4500
	twoway (scatter APCn x`var'), name(APC`var', replace) legend(off) graphregion(fcolor(white)) plotregion(lcolor(black)) xtitle("") ytitle("") xlabel(1 "`lab1'" 21 "`lab21'"  41 "`lab41'"  61 "`lab61'"  81 "`lab81'" 100 "`lab100'", labsize(large)) ylabel(,labsize(large))
	graph export "$graphs/fig2_APC_`var'.pdf", as(pdf) replace

	** Text file for paper export 
	gen index=_n
	keep index APCn
	order index APCn
	export delimited using "$graphs/txt/fig2_APC_`var'_CSES2017.txt", delimiter(tab) novarnames replace
	...
	*Export average value of exposure measure in percentile, for xtick labels
	...
	export delimited using "$graphs/txt/fig2_pc_`var'_CSES2017.txt", delimiter(tab) replace	
...
**** Apply the program to the three redistribution channels on Jappelli and Pistaferri controls for APC 
...
quietly{ 	
	...
	noi di "Table 6 for `redc', CSES 2017" 
